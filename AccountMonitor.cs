    ...
    private void WriteSnapshot()
    {
        try
        {
            ...
            foreach (var pos in Positions)
            {
                ...
                string sl = pos.StopLoss.HasValue
                    ? pos.StopLoss.Value.ToString("F5")
                    : "null";
                string tp = pos.TakeProfit.HasValue
                    ? pos.TakeProfit.Value.ToString("F5")
                    : "null";
                ...
                posJson.AppendFormat(
                    @"{{""id"":{0},""symbol"":""{1}"",""direction"":""{2}""," +
                    @"""lots"":{3},""entry_price"":{4},""current_price"":{5}," +
                    @"""pnl"":{6:F2},""pips"":{7:F1},""sl"":{8},""tp"":{9}," +
                    @"""label"":""{10}"",""open_time"":""{11:yyyy-MM-ddTHH:mm:ss}""}}",
                    pos.Id,
                    pos.SymbolName,
                    pos.TradeType,
                    pos.Quantity.ToString("F2"),
                    pos.EntryPrice.ToString("F5"),
                    currentPrice.ToString("F5"),
                    pos.NetProfit,
                    pos.Pips,
                    sl,
                    tp,
            ...
            foreach (var trade in History)
            {
                ...
                if (count < 100)
                {
                    ...
                    histJson.AppendFormat(
                        @"{{""id"":{0},""symbol"":""{1}"",""direction"":""{2}""," +
                        @"""lots"":{3},""entry_price"":{4},""close_price"":{5}," +
                        @"""pnl"":{6:F2},""pips"":{7:F1}," +
            ...
            string json = string.Format(
                ...
                @"""account_number"":""{1}"",""currency"":""{2}""," +
                @"""balance"":{3:F2},""equity"":{4:F2}," +
                @"""unrealized_pnl"":{5:F2},""realized_pnl"":{6:F2}," +
                @"""open_positions"":{7},""total_trades"":{8}," +
                @"""wins"":{9},""losses"":{10},""win_rate"":{11:F1}," +
                @"""positions"":{12},""history"":{13}}}",
                DateTime.UtcNow,
                Account.Number,
                currency,
                balance,
            ...
            // ── Scrittura file (System.IO qualificato per evitare ambiguità) ──
            if (!System.IO.Directory.Exists(OutputFolder))
                System.IO.Directory.CreateDirectory(OutputFolder);

            string outputPath = System.IO.Path.Combine(OutputFolder, "snapshot.json");
